...
function getDaysBetweenSolar(year, month, day, year1, month1, day1) {
  ...
  let initDate = year + '-' + (month < 10 ? '0' + (month + 1) : month + 1) + '-' + (day < 10 ? '0' + day : day);
  let currDate = year1 + '-' + (month1 < 10 ? '0' + (month1 + 1) : month1 + 1) + '-' + (day1 < 10 ? '0' + day1 : day1);

  let initDateForMat = "\"" + initDate + "\"";
  let currDateForMat = "\"" + currDate + "\"";

  var date = new Date(initDateForMat).getTime();
  var date1 = new Date(currDateForMat).getTime();

  if (/^\d+.\d+$/.test((date1 - date) / 86400000)) {
    return (parseInt((date1 - date) / 86400000));
  } else {
    const date2 = new Date(year, month, day).getTime();
    const date3 = new Date(year1, month1, day1).getTime();
    return (date3 - date2) / 86400000;
  }
